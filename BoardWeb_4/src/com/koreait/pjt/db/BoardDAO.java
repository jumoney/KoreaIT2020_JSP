...
public class BoardDAO {
	...
	public static List<BoardDomain> selBoardList(BoardDomain param) {
		...
		String sql = " SELECT A.* FROM ( " + " SELECT ROWNUM as RNUM, A.* FROM ( "
				...
				+ "         , nvl(C.cnt, 0) as like_cnt " + "			, nvl(D.cnt, 0) as cmt_cnt "
				+ "			, DECODE(E.i_board, null, 0, 1) as yn_like " + " 		FROM t_board4 A "
				+ " 		INNER JOIN t_user B " + " 		ON A.i_user = B.i_user" + "			LEFT JOIN ( "
				...
				+ "			) D " + "			ON A.i_board = D.i_board " + "			LEFT JOIN ( "
				+ "				 SELECT i_board FROM t_board4_like WHERE i_user = ? " + "			) E "
				+ "			ON A.i_board = E.i_board " + " 		WHERE  ";
	...
	public static BoardDomain selBoard(final BoardVO param) {
		...
		String sql = " SELECT B.profile_img, B.nm, A.i_user "
				...
				+ " , DECODE(C.i_user, null, 0, 1) as yn_like, nvl(D.cnt, 0) as like_cnt" + " FROM t_board4 A "
				+ " INNER JOIN t_user B " + " ON A.i_user = B.i_user " + " LEFT JOIN t_board4_like C "
				+ " ON A.i_board = C.i_board " + " AND C.i_user = ? " + " LEFT JOIN ( "
				+ " 	SELECT i_board, count(i_board) as cnt FROM t_board4_like " + " 	WHERE i_board = ? "
				+ " 	GROUP BY i_board " + " ) D " + " ON A.i_board = D.i_board " + " WHERE A.i_board = ? ";
