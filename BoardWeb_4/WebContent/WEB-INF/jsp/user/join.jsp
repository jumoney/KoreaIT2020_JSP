	...
	<div id="container">
		<div>
			<form id="frm" action="/join" method="post" onsubmit="return chk()">
				...
				<div><label><input type="text" name="nm" placeholder="이름" value="${data.nm}" required></label></div>
				<div><label><input type="email" name="e_mail" placeholder="이메일" value="${data.e_mail}"></label></div>
				<div><label><input type="submit" value ="회원가입"></label></div>
			</form>
		</div>
	</div>
	<script>
		...
		function chk() {
			if (frm.user_id.value.length < 5) {
				...
				if (!e_mail.test(frm.e_mail.value)) {
					alert('이메일을 확인해 주세요.');
					frm.e_mail.focus();
					return false;
				}
